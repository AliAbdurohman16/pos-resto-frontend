...
const BottomNav = () => {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-[#262626] flex justify-around p-2 h-16">
            ...
            <button disabled={isActive("/tables") || isActive("/menu")} onClick={openModal} className="absolute bottom-6 bg-[#F6B100] text-[#f5f5f5] rounded-full p-3 items-center"><BiSolidDish size={30} /></button>

            <Modal isOpen={isModalOpen} onClose={closeModal} title="Create Order">
                <div>
                    <label className="block text-[#ababab] mb-2 text-sm font-medium">Customer Name</label>
                    <div className="flex items-center rounded-lg py-3 px-4 bg-[#1f1f1f]">
                        <input type="text" name="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter customer name" id="" className="bg-transparent flex-1 text-white focus:outline-none" />
                    </div>
                </div>
                <div>
                    <label className="block text-[#ababab] mb-2 text-sm font-medium mt-3">Customer Phone</label>
                    <div className="flex items-center rounded-lg py-3 px-4 bg-[#1f1f1f]">
                        <input type="number" name="phone" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="08234567890" id="" className="bg-transparent flex-1 text-white focus:outline-none" />
                    </div>
                </div>
                <div>
