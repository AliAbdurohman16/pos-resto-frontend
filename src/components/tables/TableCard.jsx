...
const TableCard = ({key, name, status, initials, seats}) => {
    ...
    return (
        <div onClick={() => handleClick(name)} key={key} className="w-[255px] bg-[#262626] hover:bg-[#2c2c2c] p-4 rounded-lg cursor-pointer">
            <div className="flex items-center justify-between px-1">
                <h1 className="text-[#f5f5f5] text-xl font-semibold">{name}</h1>
                <p className={`${status === "Booked" ? "text-green-600 bg-[#2e4a40]" : "text-yellow-500 bg-[#664a04]"} px-2 py-1 rounded-lg`}>{status}</p>
            </div>
